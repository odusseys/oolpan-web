...
export function useArabicTrainer() {
  const [statsByKey, setStatsByKey] = useState<Map<string, ReviewStat>>(() => new Map());
  const [currentCard, setCurrentCard] = useState<DirectedFlashcard | null>(null);
  ...
  const review = useCallback(
    (result: ReviewResult) => {
      ...
      setStatsByKey((currentStats) => {
        ...
        setIsRevealed(false);
        void saveReviewStat(updatedStat).catch((error: unknown) => {
...
function nextCard(statsByKey: Map<string, ReviewStat>, previousKey: string | null) {
  const card = sampleFlashcard(DIRECTED_FLASHCARDS, statsByKey, previousKey);

  return card ? withRandomArabicPrompt(card) : null;
}

function getProgressSummary(statsByKey: Map<string, ReviewStat>) {
